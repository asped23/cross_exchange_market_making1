...
class CrossExchangeMarketMakingStrategy(StrategyPyBase):
    ...
    def did_fail_order(self, order_failed_event: MarketOrderFailureEvent):
        ...
        if order_failed_event.order_id in self._taker_to_maker_order_ids.keys():
            self.notify_hb_app_with_timestamp(
                f"Order with value inferior to taker exchange minimum : Continue")
            ...
            if order_id in self._taker_to_maker_order_ids:
                ...
                # Suppression de l'ordre taker des dictionnaires
                del self._taker_to_maker_order_ids[order_id]
                self.logger().info(f"Removed taker order {order_id} from _taker_to_maker_order_ids.")
    
                # Suppression des quantités remplies pour le taker
                if maker_order_id in self._taker_filled_quantities:
                    try:
                        del self._taker_filled_quantities[maker_order_id][order_id]
                        self.logger().info(f"Removed filled quantity for taker order {order_id}.")
                        if not self._taker_filled_quantities[maker_order_id]:
                            del self._taker_filled_quantities[maker_order_id]
                            self.logger().info(f"Removed maker order {maker_order_id} from _taker_filled_quantities.")
                    except KeyError:
                        self.logger().warning(f"Failed to remove {order_id} from _taker_filled_quantities.")
    
                # Nettoyage des timestamps
                if order_id in self._taker_order_timestamps:
                    del self._taker_order_timestamps[order_id]
                    self.logger().info(f"Removed timestamp for taker order {order_id}.")
    
                # Nettoyage des ordres à annuler
                if order_id in self._orders_to_cancel:
                    self._orders_to_cancel.remove(order_id)
                    self.logger().info(f"Removed {order_id} from _orders_to_cancel.")
    
                # Vérification si tous les ordres taker associés à l'ordre maker ont échoué ou ont été complétés
                ...
                if len(active_taker_ids) == 0:
                    # Si aucun autre ordre taker actif, on supprime l'ordre maker des dictionnaires associés
                    maker_order_ids = [order_id for market, limit_order, order_id in self.active_maker_limit_orders]
                    if maker_order_id not in maker_order_ids:
                        try:
                            del self._maker_to_taker_order_ids[maker_order_id]
                            del self._maker_to_hedging_trades[maker_order_id]
                            self.logger().info(f"Removed maker order {maker_order_id} from _maker_to_taker_order_ids and _maker_to_hedging_trades.")
                        except KeyError:
                            self.logger().warning(f"Failed to remove maker order {maker_order_id} from tracking dictionaries.")
    
                # Nettoyage des ongoing hedging
                try:
                    self.del_order_from_ongoing_hedging(order_id)
                    self.logger().info(f"Removed taker order {order_id} from ongoing hedging.")
                except KeyError:
                    self.logger().warning(f"Ongoing hedging not found for order id {order_id}.")
    
                # Nettoyage des évènements de fill associés (buy ou sell selon le type d'ordre maker)
                original_order = self._sb_order_tracker.get_limit_order(market_pair.maker, maker_order_id)
    
                if original_order.is_buy:
                    # Nettoyage des évènements de fill pour les sell events (achat sur le maker)
                    try:
                        fill_events = []
                        for fill_event in self._order_fill_sell_events[market_pair]:
                            if self.is_fill_event_in_ongoing_hedging(fill_event):
                        ...
                        if len(self._order_fill_sell_events[market_pair]) == 0:
                            del self._order_fill_sell_events[market_pair]
                            self.logger().info(f"All sell fill events cleaned for market pair {market_pair}.")
                    except KeyError:
                        self.logger().warning(f"No sell fill events found for market pair {market_pair}.")
    
                else:
                    # Nettoyage des évènements de fill pour les buy events (vente sur le maker)
                    try:
                        fill_events = []
                        for fill_event in self._order_fill_buy_events[market_pair]:
                            if self.is_fill_event_in_ongoing_hedging(fill_event):
                        ...
                        if len(self._order_fill_buy_events[market_pair]) == 0:
                            del self._order_fill_buy_events[market_pair]
                            self.logger().info(f"All buy fill events cleaned for market pair {market_pair}.")
                    except KeyError:
                        self.logger().warning(f"No buy fill events found for market pair {market_pair}.")
    
            # Si l'ordre taker n'est pas trouvé, log un avertissement
            else:
                self.logger().warning(f"Taker order {order_id} not found in _taker_to_maker_order_ids.")
    
            # Déclenche la prochaine itération de placement des ordres maker
            self.logger().info("Placing new maker orders after taker order failure.")
            self.trigger_new_maker_orders(market_pair)
    ...
    def did_complete_buy_order(self, order_completed_event: BuyOrderCompletedEvent):
        ...
        if market_pair is not None:
            ...
            if order_id in self._taker_to_maker_order_ids.keys():
                limit_order_record = self._sb_order_tracker.get_limit_order(market_pair.taker, order_id)
                self.log_with_clock(
                    logging.INFO,
                ...
                self.notify_hb_app_with_timestamp(
                    ...
                    f" {limit_order_record.price} {order_completed_event.quote_asset}) is filled."
                )
                maker_order_id = self._taker_to_maker_order_ids[order_id]
